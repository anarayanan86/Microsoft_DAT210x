...
def doKMeans(data, clusters = 0):
  #
  # TODO: Do the KMeans clustering here, passing in the # of clusters parameter and fit it against your data. Then,
  # return a tuple containing the cluster centers and the labels
  #
  # .. your code here ..
...
#
# TODO: Load up the dataset. It has may or may not have nans in it. Make sure you catch them and destroy them, by setting them to '0'.
...
#
# TODO: As instructed, get rid of the 'Channel' and 'Region' columns, since you'll be investigating as if this were a single
# location wholesaler, rather than a national / international one. Leaving these fields in here would cause KMeans to
# examine and give weight to them.
#
# .. your code here ..
df.drop(labels = ['Channel', 'Region'], axis = 1, inplace = True)

#
# TODO: Before unitizing / standardizing / normalizing your data in preparation for K-Means, it's a good idea to get a quick
# peek at it. You can do this using the .describe() method, or even by using the built-in pandas df.plot.hist()
#
# .. your code here ..
print df.describe()
df.plot.hist()
...
#
# INFO: Having checked out your data, you may have noticed there's a pretty big gap between the top customers in each feature
# category and the rest. Some feature scaling algos won't get rid of outliers for you, so it's a good idea to handle that
# manually---particularly if your goal is NOT to determine the top customers. After all, you can do that with a simple Pandas
# .sort_values() and not a machine learning clustering algorithm. From a business perspective, you're probably more
# interested in clustering your +/- 2 standard deviation customers, rather than the creme dela creme, or bottom of the barrel'ers
#
...
#
# INFO Drop rows by index. We do this all at once in case there is a collision. This way, we don't end up dropping more rows
# than we have to, if there is a single row that satisfies the drop for multiple columns.
...
# The answer depends on what you're trying to accomplish. For instance, although all the units are the same (generic money unit),
# the price per item in your store isn't. There may be some cheap items and some expensive ones. If your goal is to find out what
# items people buy tend to buy together but you didn't unitize properly before running kMeans, the contribution of the lesser priced
...
#
# Suffice to say, at the end of the day, you're going to have to know what question you want answered and what data you have
# available in order to select the best method for your purpose. Luckily, SKLearn's interfaces are easy to switch out
# so in the mean time, you can experiment with all of them and see how they alter your results.
#
#
...
#
# TODO: Un-comment just ***ONE*** of lines at a time and see how that alters your results.
...
#
# INFO: Sometimes people perform PCA before doing KMeans, so that KMeans only operates on the most meaningful features.
# In our case, there are so few features that doing PCA ahead of time isn't really necessary, and you can do KMeans in
# feature space. But keep in mind you have the option to transform your data to bring down its dimensionality. If you take
# that route, then your Clusters will already be in PCA-transformed feature space, and you won't have to project them
...
# Lab Question:
# 
# Which of SciKit-Learn's preprocessors causes the principal components to fan out as much as possible in an arrangement
# unlike the others?
# Normalize